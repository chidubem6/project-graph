...
- 04 Project dialogue (`context/feature-specs/04-project-dialogue`)
  ...
  - [x] `hooks/use-project-dialogs.ts` — single owner of dialog kind (`create` | `rename` | `delete`), target project, shared name input, derived slug and `isSubmitting`. Closing leaves form values in place so they don't flicker during the exit animation; each open resets what it needs. `submit()` only flips the loading flag and closes — persistence lands with the project API slice.
  - [x] `lib/project-slug.ts` — `toProjectSlug()`: lowercase, strip diacritics (NFD), non-alphanumerics to hyphens, trim hyphens. Checked against "Payments API v2" → `payments-api-v2`, "Ünïcode Test" → `unicode-test`, "###" → `""`.
...
- Spec 04 asks the hook for loading state, but nothing is awaited yet, so `submit()` sets `isSubmitting` and closes immediately. The flag exists so the buttons already have their disabled wiring when persistence arrives.
